...
@section('title', $machine->name . ' ポット' . $potNumber . ' | 加工機工具管理システム')
@section('page-title', isset($pot) ? 'ポット編集' : 'ポット登録')
    ...
    <div class="form-card">
        <div class="form-card-header" style="padding:20px 24px 0;">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
                <div class="magazine-show-icon" style="width:44px;height:44px;border-radius:12px;background:rgba(79,142,247,0.1);color:var(--color-primary);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                ...
                <div>
                    <p style="font-size:0.8rem;color:var(--color-text-muted);margin-bottom:2px;">{{ $machine->name }}{{ $machine->machine_number ? ' No.'.$machine->machine_number : '' }}</p>
                    <h3 style="font-size:1.1rem;font-weight:700;color:var(--color-text);">ポット {{ $potNumber }} の工具{{ isset($pot) ? '編集' : '登録' }}</h3>
                </div>
            </div>
        </div>
        ...
        <form id="potForm"
              method="POST"
              action="{{ isset($pot) ? route('admin.magazines.updatePot', [$machine, $pot]) : route('admin.magazines.storePot', $machine) }}"
              style="padding:0 24px 24px;">
            @csrf
            @if(isset($pot)) @method('PUT') @endif
            <input type="hidden" name="pot_number" value="{{ $potNumber }}">
            <input type="hidden" name="from_standby_id" id="fromStandbyId" value="">
            ...
            <div style="display:flex;align-items:flex-start;gap:8px;background:rgba(79,142,247,0.06);border:1px solid rgba(79,142,247,0.25);border-radius:8px;padding:10px 14px;margin-bottom:20px;">
                ...
                <p style="font-size:0.82rem;color:var(--color-primary);line-height:1.6;margin:0;">
                    最初に登録した工具名がポットカードに表示されます。<br>
                    カテゴリーを選択すると工具名が表示されます。工具を選択すると次の行が自動で追加されます。
                </p>
            </div>

            {{-- 待機工具セットから追加 --}}
            @if ($standbySets->isNotEmpty())
            <div class="standby-section">
                <button type="button" class="standby-toggle" onclick="toggleStandby(this)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                        <polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>
                    </svg>
                    待機工具セットから追加
                    <span class="standby-count">{{ $standbySets->count() }}</span>
                    <svg class="standby-arrow" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-left:auto;transition:transform 0.2s;">
                        <polyline points="6 9 12 15 18 9"/>
                    </svg>
                </button>
                <div class="standby-list" style="display:none">
                    @foreach ($standbySets as $set)
                    <button type="button" class="standby-item"
                            data-id="{{ $set->id }}"
                            data-tools="{{ json_encode($set->tools->map(fn($t) => ['cat' => $t->tool_category_id, 'id' => $t->id])) }}"
                            onclick="applyStandbySet(this)">
                        <span class="standby-set-info">
                            <span class="standby-item-name">{{ $set->tools->first()?->name ?? '(工具なし)' }}</span>
                            <span class="standby-item-meta">
                                {{ $set->machine->name ?? '機械未設定' }} ／ {{ $set->tools->count() }} 工具
                            </span>
                        </span>
                        <span class="standby-item-add">このセットを使用</span>
                    </button>
                    @endforeach
                </div>
            </div>
            ...
            <div class="form-actions" style="padding-top:8px;">
                <a href="{{ route('admin.magazines.show', $machine) }}" class="btn-cancel">キャンセル</a>
...
const currentTools    = @json($currentTools ?? []);
...
// 待機セットをすべて展開してフォーム行に追加
function applyStandbySet(btn) {
    ...
    tools.forEach((t, i) => {
        container.appendChild(buildRow(i, t.cat, t.id));
    });
    container.appendChild(buildRow(tools.length));
    updateRemoveBtns();

    // 待機セットIDをセット（storePot 時に削除対象として使用）
    document.getElementById('fromStandbyId').value = setId;

    // ボタンを使用済み状態に
    document.querySelectorAll('.standby-item').forEach(b => {
        b.classList.remove('standby-item-added');
        b.querySelector('.standby-item-add').textContent = 'このセットを使用';
        b.disabled = false;
    });
    btn.classList.add('standby-item-added');
    btn.querySelector('.standby-item-add').textContent = '使用中';
    btn.disabled = true;
}

function toggleStandby(btn) {
    const list  = btn.nextElementSibling;
    const arrow = btn.querySelector('.standby-arrow');
...
// 送信前に空行を除外
document.getElementById('potForm').addEventListener('submit', function(e) {
...
(function init() {
    ...
    if (currentTools.length > 0) {
        currentTools.forEach((t, i) => container.appendChild(buildRow(i, t.category_id, t.id)));
        container.appendChild(buildRow(currentTools.length));
    } else {
