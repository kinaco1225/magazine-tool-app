...
@section('title', '工具登録 | 加工機工具管理システム')
@section('page-title', '工具管理')

@section('content')

    <div class="content-header">
        @if (request('tool_category_id'))
            <a href="{{ route('admin.tools.category', request('tool_category_id')) }}" class="btn-back">
    ...
    <div class="form-card">
        ...
        <form action="{{ route('admin.tools.store') }}" method="POST" class="form">
            @csrf
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        ...
                        <select id="tool_category_id" name="tool_category_id" required class="{{ $errors->has('tool_category_id') ? 'is-invalid' : '' }}">
                            <option value="">カテゴリーを選択</option>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ old('tool_category_id', request('tool_category_id')) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <label style="display:inline-flex;align-items:center;gap:10px;cursor:pointer;">
                        <span style="position:relative;display:inline-block;width:44px;height:24px;flex-shrink:0;">
                            <input type="checkbox" id="manages_stock" name="manages_stock" value="1"
                                {{ old('manages_stock', '1') !== '0' ? 'checked' : '' }}
                                style="position:absolute;opacity:0;width:0;height:0;"
                                onchange="updateStockToggle(this)">
                            <span id="manages_stock_track" style="position:absolute;inset:0;border-radius:12px;transition:background 0.2s;background:var(--color-primary,#4f8ef7);"></span>
                            <span id="manages_stock_thumb" style="position:absolute;top:2px;width:20px;height:20px;background:#fff;border-radius:50%;box-shadow:0 1px 3px rgba(0,0,0,.2);transition:left 0.2s;left:22px;"></span>
                        </span>
                        <span id="manages_stock_label" style="font-size:0.9rem;color:var(--color-text);">在庫管理する</span>
                    </label>
                </div>
            </div>
            ...
            <div class="form-actions">
                <a href="{{ request('tool_category_id') ? route('admin.tools.category', request('tool_category_id')) : route('admin.tools.index') }}" class="btn-cancel">キャンセル</a>
...
// 初期状態を反映
(function() {
    const el = document.getElementById('manages_stock');
    if (el && !el.checked) updateStockToggle(el);
})();
</script>
@endpush
